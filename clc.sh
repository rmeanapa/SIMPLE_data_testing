mkdir -p clc; cd clc
simple_exec prg=new_project projname=clc dir=./ > LOG 2>&1
filetab_movs.pl /mnt/beegfs/elmlund/testing-datasets/CLC/20220920_220725_wt_CLC7_OSTM1_0_1LMNG_2_5ATP_thinner_ice/movies/ 50 >> LOG 2>&1
echo " >>> PROGRAM: import_movies" >> LOG 2>&1
simple_exec prg=import_movies ctf=yes cs=2.7 fraca=0.1 kv=300 smpd=0.693 filetab=movies.txt >> LOG 2>&1
echo " >>> PROGRAM: motion_correct" >> LOG 2>&1
#simple_exec prg=motion_correct nparts=5 nthr=8 gainref= total_dose=55.72 smpd_downscale=1.3 >> LOG 2>&1
simple_exec prg=motion_correct nparts=5 nthr=8 total_dose=55.72 smpd_downscale=1.3 >> LOG 2>&1
echo " >>> PROGRAM: ctf_estimate" >> LOG 2>&1
simple_exec prg=ctf_estimate nparts=5 nthr=8 projfile=2_motion_correct/clc.simple >> LOG 2>&1
filetab_mrc.pl 2_motion_correct/ >> LOG 2>&1
echo " >>> PROGRAM: pick" >> LOG 2>&1
simple_exec prg=pick picker=segdiam projfile=3_ctf_estimate/clc.simple nparts=5 nthr=8 >> LOG 2>&1
echo " >>> PROGRAM: extract" >> LOG 2>&1
simple_exec prg=extract box=256 nparts=5 nthr=8 projfile=4_pick/clc.simple >> LOG 2>&1
...
simple_exec prg=abinitio3D_cavgs pgrp=c2 mskdiam=180 nthr=40 >> LOG 2>&1
echo " >>> PROGRAM: abinitio3D" >> LOG 2>&1
simple_exec prg=abinitio3D pgrp=c2 mskdiam=180 nthr=8 nparts=10 cavg_ini_ext=yes >> LOG 2>&1
echo " >>> PROGRAM: refine3D_auto" >> LOG 2>&1
simple_exec prg=refine3D_auto pgrp=c2 mskdiam=180 nparts=10 nthr=8 >> LOG 2>&1
